...
constexpr double GYRO_BIAS_STD = 1/180.0 * M_PI;
constexpr double INIT_VEL_STD = 5.0;
constexpr double INIT_PSI_STD = 45.0/180.0 * M_PI;
constexpr double GPS_POS_STD = 3.0;
constexpr double LIDAR_RANGE_STD = 3.0;
constexpr double LIDAR_THETA_STD = 0.02;
bool lidar_init = false;
constexpr double THRESHOLD = 7.0;
// ----------------------------------------------------------------------- //
...
{ 
    ...
    if (lidar_init)
    {
        ...
        if (meas.id != -1 && map_beacon.id != -1)
        {
            ...
            double epsilon = y.transpose() * Py.inverse() * y;
/*             if (epsilon < THRESHOLD)
            ...
            {
                std::cout<<"rejecting Lidar measurement"<<std::endl;
            } */
            state = state + K*y;
    ...
    else
    {
        ...
        double psi = atan2(dy,dx) - meas.theta ; 
        state(2) = psi;
        std::cout << "init psi: "<<psi<<std::endl;
        setState(normaliseState(state));
        lidar_init = true;
    }
}
...
void KalmanFilter::predictionStep(GyroMeasurement gyro, double dt)
{
    if (isInitialised())
    {
        ...
        Q(2,2) = GYRO_BIAS_STD * GYRO_BIAS_STD;
...
void KalmanFilter::handleGPSMeasurement(GPSMeasurement meas)
{
    ...
    if(isInitialised())
    {
        ...
        {
            std::cout<<"rejecting GPS measurement"<<std::endl;
        }
    ...
    else
    {
        ...
        std::cout << "init by GPS : "<<state<<std::endl;
